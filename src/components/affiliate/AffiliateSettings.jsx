...
const AffiliateSettings = () => {
  const { affiliateData, updateStoreSettings, isAffiliate, loading } = useAffiliate();
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {message && (
          <div className={`p-4 mb-6 rounded-lg ${message.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {message.type === 'success' && <CheckCircle className="inline-block h-5 w-5 mr-2" />}
            {message.text}
          </div>
        )}
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                This will be used in your store URL: {window.location.origin}/affiliate/{formData.storeName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}
              </p>
            </div>
